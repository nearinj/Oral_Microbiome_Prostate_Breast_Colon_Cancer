...
Breast_data$ID <- gsub("CS","",Breast_data$CaseID)
Breast_data$ID <- as.numeric(Breast_data$ID)

Breast_data$Extraction_numeric <- gsub("Extraction\\.","",Breast_data$Extraction_Number)
Breast_data$Extraction_numeric <- as.numeric(Breast_data$Extraction_numeric)
Breast_data$ID <- as.numeric(gsub("CS", "", Breast_data$CaseID))

Breast_res <- MiRKAT(y = Breast_data$Case.Control_num, X=Breast_data$Extraction_Number,
                      Ks=Breast_Ks, out_type = "D", returnR2 = TRUE)
Breast_res

covars <- cbind(Breast_data$Extraction_numeric, Breast_data$A_SDC_AGE_CALC)
...
Breast_data_noNA <- Breast_data[-which(is.na(Breast_data$PM_WAIST_HIP_RATIO)),]
Breast_data_noNA <- Breast_data_noNA[-which(is.na(Breast_data_noNA$NUT_VEG_DAY_QTY)),]  

#need to remake the distance metrics and kernels -.-

covars <- cbind(Breast_data_noNA$Extraction_numeric,
                Breast_data_noNA$A_SDC_AGE_CALC,
                Breast_data_noNA$PM_STANDING_HEIGHT_AVG,
                Breast_data_noNA$PM_WAIST_HIP_RATIO,
                Breast_data_noNA$NUT_VEG_DAY_QTY)
...
Prostate_data$Extraction_numeric <- gsub("Extraction\\.","",Prostate_data$Extraction_Number)
Prostate_data$Extraction_numeric <- as.numeric(Prostate_data$Extraction_numeric)

Prostate_res <- MiRKAT(y = Prostate_data$Case.Control_num, X=Prostate_data$Extraction_Number,
                      Ks=Prostate_Ks, out_type = "D", returnR2 = TRUE)
Prostate_res

covars <- cbind(Prostate_data$Extraction_numeric, Prostate_data$A_SDC_AGE_CALC)
...
Prostate_data_noNA <- Prostate_data[-which(is.na(Prostate_data$PM_WAIST_HIP_RATIO)),]

#need to remake the distance metrics and kernels -.-

covars <- cbind(Prostate_data_noNA$Extraction_numeric,
                Prostate_data_noNA$A_SDC_AGE_CALC,
                Prostate_data_noNA$PM_STANDING_HEIGHT_AVG,
                Prostate_data_noNA$PM_WAIST_HIP_RATIO,
                Prostate_data_noNA$NUT_VEG_DAY_QTY)
...
CRC_data$Extraction_numeric <- gsub("Extraction\\.","",CRC_data$Extraction_Number)
CRC_data$Extraction_numeric <- as.numeric(CRC_data$Extraction_numeric)

CRC_res <- MiRKAT(y = CRC_data$Case.Control_num, X=CRC_data$Extraction_Number,
                      Ks=CRC_Ks, out_type = "D", returnR2 = TRUE)
CRC_res

covars <- cbind(CRC_data$Extraction_numeric, CRC_data$A_SDC_AGE_CALC, CRC_data$A_SDC_GENDER)
...
covars <- cbind(CRC_data$Extraction_numeric,
                CRC_data$A_SDC_AGE_CALC,
                CRC_data$PM_STANDING_HEIGHT_AVG,
                CRC_data$PM_WAIST_HIP_RATIO,
                CRC_data$NUT_VEG_DAY_QTY)



CRC_res_all <- MiRKAT(y = CRC_data$Case.Control_num, X=covars,
                      Ks=CRC_Ks, out_type = "D", returnR2 = TRUE)
CRC_res_all
```