...
OTU <- phyloseq::otu_table(asv_breast, taxa_are_rows = T)
sampledata <- phyloseq::sample_data(Breast_data, errorIfNULL = T)
phylo <- phyloseq::merge_phyloseq(OTU, sampledata)

Breast_ASV_test <- differentialTest(formula = ~ Case.Control,
                                    phi.formula = ~ Case.Control,
                                    formula_null= ~ 1,
                                    phi.formula_null = ~ Case.Control,
                                    test="Wald", data=phylo,
                                    boot=F,
                                    fdr_cutoff=0.1)

saveRDS(Breast_ASV_test, "~/Private/CHAPTER_4/Prospec/ATP/data/Breast/ASV_corncob.RDS")
```

## Genera

```{r}
OTU <- phyloseq::otu_table(genera_breast, taxa_are_rows = T)
sampledata <- phyloseq::sample_data(Breast_data, errorIfNULL = T)
phylo <- phyloseq::merge_phyloseq(OTU, sampledata)

Breast_genera_test <- differentialTest(formula = ~ Case.Control,
                                    phi.formula = ~ Case.Control,
                                    formula_null= ~ 1,
                                    phi.formula_null = ~ Case.Control,
                                    test="Wald", data=phylo,
                                    boot=F,
                                    fdr_cutoff=0.1)
saveRDS(Breast_genera_test, "~/Private/CHAPTER_4/Prospec/ATP/data/Breast/Genera_corncob.RDS")
```
...
OTU <- phyloseq::otu_table(asv_prostate, taxa_are_rows = T)
sampledata <- phyloseq::sample_data(Prostate_data, errorIfNULL = T)
phylo <- phyloseq::merge_phyloseq(OTU, sampledata)

Prostate_ASV_test <- differentialTest(formula = ~ Case.Control,
                                    phi.formula = ~ Case.Control,
                                    formula_null= ~ 1,
                                    phi.formula_null = ~ Case.Control,
                                    test="Wald", data=phylo,
...
saveRDS(Prostate_ASV_test, "~/Private/CHAPTER_4/Prospec/ATP/data/Prostate/ASV_corncob.RDS")
```

## Genera
```{r}
OTU <- phyloseq::otu_table(genera_prostate, taxa_are_rows = T)
sampledata <- phyloseq::sample_data(Prostate_data, errorIfNULL = T)
phylo <- phyloseq::merge_phyloseq(OTU, sampledata)

Prostate_genera_test <- differentialTest(formula = ~ Case.Control,
                                    phi.formula = ~ Case.Control,
                                    formula_null= ~ 1,
                                    phi.formula_null = ~ Case.Control,
                                    test="Wald", data=phylo,
...
saveRDS(Prostate_genera_test, "~/Private/CHAPTER_4/Prospec/ATP/data/Prostate/Genera_corncob.RDS")
```
...
OTU <- phyloseq::otu_table(asv_colon, taxa_are_rows = T)
sampledata <- phyloseq::sample_data(CRC_data, errorIfNULL = T)
phylo <- phyloseq::merge_phyloseq(OTU, sampledata)

Colon_ASV_test <- differentialTest(formula = ~ Case.Control,
                                    phi.formula = ~ Case.Control,
                                    formula_null= ~ 1,
                                    phi.formula_null = ~ Case.Control,
                                    test="Wald", data=phylo,
                                    boot=F,
                                    fdr_cutoff=0.1)
saveRDS(Colon_ASV_test, "~/Private/CHAPTER_4/Prospec/ATP/data/Colon/ASV_corncob.RDS")
```

## Genera
```{r}
OTU <- phyloseq::otu_table(genera_colon, taxa_are_rows = T)
sampledata <- phyloseq::sample_data(CRC_data, errorIfNULL = T)
phylo <- phyloseq::merge_phyloseq(OTU, sampledata)

Colon_genera_test <- differentialTest(formula = ~ Case.Control,
                                    phi.formula = ~ Case.Control,
                                    formula_null= ~ 1,
                                    phi.formula_null = ~ Case.Control,
                                    test="Wald", data=phylo,
...
saveRDS(Colon_genera_test, "~/Private/CHAPTER_4/Prospec/ATP/data/Colon/Genera_corncob.RDS")
```